    ...
    : ROI(pos, size, parent)
{
    ...
    if (0) {
        ...
        // 添加四个角的 scale handle（中心缩放）
        addScaleHandle({0.0, 0.0}, center);  // 左下角
        addScaleHandle({1.0, 0.0}, center);  // 右下角
        addScaleHandle({0.0, 1.0}, center);  // 左上角
        addScaleHandle({1.0, 1.0}, center);  // 右上角

            // 添加四个边中点的 scale handle（中心缩放）
            addScaleHandle({1.0, 0.5}, {0.5, 0.5});  // 右中
            addScaleHandle({0.5, 1.0}, {0.5, 0.5}); // 上中
            addScaleHandle({0.0, 0.5}, {0.5, 0.5}); // 左中
            addScaleHandle({0.5, 0.0}, {0.5, 0.5}); // 下中
        addRotateHandle({0.5, -0.1}, center);  // 旋转手柄

    } else { // 非中心缩放 (对角缩放)
        // 添加四个角的 scale handle（对角缩放）
        addScaleHandle({0.0, 0.0}, {1.0, 1.0});  // 左下角, 锚点: 右上角
