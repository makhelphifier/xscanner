...
void MotionHelper::checkPath(const QString& dirName)
{
    //相对路径需要补全完整路径
    ...
    {
        path.replace(".", "");
        path = MotionHelper::appPath() + path;
    }
...
void MotionHelper::initDialog(QFileDialog* dialog, const QString& title, const QString& acceptName, const QString& dirName, bool native, int width, int height)
{
    //设置标题
...
QString MotionHelper::getDialogResult(QFileDialog* dialog)
{
    QString result;
...
QString MotionHelper::getOpenFileName(const QString& filter, const QString& dirName, const QString& fileName, bool native, int width, int height)
{
    QFileDialog dialog;
...
QString MotionHelper::getSaveFileName(const QString& filter, const QString& dirName, const QString& fileName, bool native, int width, int height)
{
    QFileDialog dialog;
...
QString MotionHelper::getExistingDirectory(const QString& dirName, bool native, int width, int height)
{
    QFileDialog dialog;
...
bool MotionHelper::checkIniFile(const QString& iniFile)
{
...
bool MotionHelper::checkJsonFile(const QString& jsonFile)
{
    QFile file(jsonFile);
    if (file.size() == 0)
    {
        qDebug() << u8"MOTIONHelper::checkJsonFile jsonFile 文件为 null";
        return false;
    }
